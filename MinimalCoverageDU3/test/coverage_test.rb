...
class CoverageTest < Test::Unit::TestCase
  ...
  def test_lines
    ...
    assert_equal(0,line.x2)
    line = lines[1]
    assert_equal(-5,line.x1)
    assert_equal(-3,line.x2)
    line = lines[2]
  ...
  def test_count
    lines = test_lines
    problem=CoverageProblem.new(1,lines)
    problem.solve
    assert_equal(0,problem.coverLines.size)
  end
